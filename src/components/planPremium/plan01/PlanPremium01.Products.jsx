        ...
        { posts ? posts.map( ( post, index ) => (
          <article key={index} className='w3-half p-1' >
            ...
            <div className="w3-row w3-white">
              ...
              <div className="w3-col s4 w3-center">
                <span>{ findItem(post.id.$t) ? findItem(post.id.$t).quantity : 0 }</span>
              </div>
              <div className="w3-col s4 w3-center">
                <button className="w3-button w3-teal w-100" onClick={ () => {
                  console.log('el coche:', car);
                  console.log(  'el coche', car.length > 0 ? car.find( c => c.id === retrievePostID(post.id.$t) ).quantity : 9  );
                  updateCar(
                    {
